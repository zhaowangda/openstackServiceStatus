...
def hoststatus(hostip):
	...
	try:
		...
		diskInfo = result[16].replace("\n","")
		memInfo = result[20].replace("\n","")
		totalProcess = result[24].replace("\n","")
		...
		strcmd2 = " update hostSchema set diskInfo='"+diskInfo+"' where hostIP='"+hostip+"'"		
		strcmd3 = " update hostSchema set memInfo='"+memInfo+"' where hostIP='"+hostip+"'"
		strcmd4 = " update hostSchema set totalProcess='"+totalProcess+"' where hostIP='"+hostip+"'"
		print strcmd4 
		result = cur.execute(strcmd)
                result = cur.execute(strcmd1)
		result = cur.execute(strcmd2)
		result = cur.execute(strcmd3)
		result = cur.execute(strcmd4)
                conn.commit()
		return "Running"
	except Exception,e:
                #print Exception,":",e
